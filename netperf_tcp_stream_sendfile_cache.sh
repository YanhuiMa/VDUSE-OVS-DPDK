...
# Variables
NETPERF_SERVER="192.168.101.2"  # Replace with the IP address of the netperf server
...
run_test() {
	...
	#echo $PACKET_SIZE >> "$NETPERF_LOG_FILE"
        numactl -m 0 -N 0 ip netns exec ns0 netperf -t TCP_STREAM -f m -H "$NETPERF_SERVER" -l "$NETPERF_TEST_DURATION" -- -m "$PACKET_SIZE" | grep "10^6bits/sec" -A 2 | tail -n 1 | awk '{print $5}' >> "$NETPERF_LOG_FILE"

	#numactl -m 0 -N 0 ip netns exec ns0 netperf -t TCP_SENDFILE -f m -H "$NETPERF_SERVER" -l "$NETPERF_TEST_DURATION" -F testfile -- -m "$PACKET_SIZE" | grep "10^6bits/sec" -A 2 | tail -n 1 | awk '{print $5}' >> "$NETPERF_LOG_FILE"

	#numactl -m 0 -N 0 ip netns exec ns0 netperf -t TCP_SENDFILE -f m -H "$NETPERF_SERVER" -l "$NETPERF_TEST_DURATION" -- -m "$PACKET_SIZE" | grep "10^6bits/sec" -A 2 | tail -n 1 | awk '{print $5}' >> "$NETPERF_LOG_FILE"
