...
run_test() {
	...
	numactl -m 0 -N 0 ip netns exec ns1 netperf -t TCP_RR -H "$NETPERF_SERVER" -l "$NETPERF_TEST_DURATION" -v 0 -- -r "$PACKET_SIZE" "$PACKET_SIZE" -o request_size,response_size,min_latency,mean_latency,max_latency,stddev_latency,transaction_rate | grep "Tran/s" -A 1 | tail -n 1 >> "$NETPERF_LOG_FILE"
